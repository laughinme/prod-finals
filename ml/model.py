...
def create_user_profiles(
    df, user_col="party_rk", category_col="category_nm", time_col="hour"
):
    df = df.copy()
    ...
    profiles_scaled = pd.DataFrame(
        scaler.fit_transform(profiles), index=profiles.index, columns=profiles.columns
    )
...
def get_matches(target_rk, profiles_df, top_n=5):
    ...
    match_results = pd.DataFrame({"party_rk": profiles_df.index, "score": similarities})
...
user_top_cats = (
    df_enriched.groupby("party_rk")["category_nm"]
