...
class MatchNotification(CreatedAtMixin, Base):
    ...
    peer_user_id: Mapped[UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE")
    )
    notification_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="match_created"
    )
...
class MessageNotification(CreatedAtMixin, Base):
    ...
    notification_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="message_received"
    )
    seen_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class LikeNotification(CreatedAtMixin, Base):
    ...
    pair_state_id: Mapped[UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("pair_states.id", ondelete="CASCADE")
    )
    notification_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="like_received"
    )
    seen_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
