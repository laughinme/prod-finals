...
class RecommendationItem(CreatedAtMixin, Base):
    ...
    batch_id: Mapped[UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("recommendation_batches.id", ondelete="CASCADE")
    )
    ...
    reason_signals: Mapped[list[dict[str, object]]] = mapped_column(
        JSON, nullable=False, default=list
    )
    category_breakdown: Mapped[list[dict[str, object]]] = mapped_column(
        JSON, nullable=False, default=list
    )
    details_available: Mapped[bool] = mapped_column(
