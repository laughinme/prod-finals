...
def upgrade() -> None:
    op.alter_column("users", "display_name", existing_type=sa.String(length=64), type_=sa.String(length=80))
    op.add_column(
        "users",
        sa.Column(
            "quiz_status",
    ...
    op.add_column("users", sa.Column("quiz_current_step_key", sa.String(length=64), nullable=True))
    op.add_column(
        "users",
        sa.Column(
    ...
    op.add_column("users", sa.Column("demo_user_key", sa.String(length=64), nullable=True))
    ...
    op.execute("UPDATE users SET has_behavioral_profile = false WHERE has_behavioral_profile IS NULL")
    ...
    op.execute("UPDATE users SET avatar_status = 'pending' WHERE avatar_status = 'pending_moderation'")
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "step_key", name="uq_onboarding_quiz_answers_user_step"),
    )
    op.create_index(
    ...
    op.drop_constraint("uq_interaction_events_actor_client", "interaction_events", type_="unique")
...
def downgrade() -> None:
    op.drop_constraint("uq_interaction_events_actor_serve_item", "interaction_events", type_="unique")
    op.create_unique_constraint(
    ...
    op.drop_index("ix_onboarding_quiz_answers_user_id", table_name="onboarding_quiz_answers")
    ...
    op.alter_column("users", "display_name", existing_type=sa.String(length=80), type_=sa.String(length=64))
    ...
    op.execute("UPDATE users SET avatar_status = 'pending_moderation' WHERE avatar_status = 'pending'")
