...
def upgrade() -> None:
    op.add_column("matches", sa.Column("source_decision_mode", sa.String(length=32), nullable=True))
    ...
    op.create_table(
        ...
        sa.Column("feed_explanation_opened", sa.Integer(), nullable=False, server_default="0"),
        ...
        sa.Column("chat_first_message_sent", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("chat_first_reply_received", sa.Integer(), nullable=False, server_default="0"),
        ...
        sa.UniqueConstraint("day", "user_source", "decision_mode", name="uq_analytics_daily_funnel_segment"),
    )
    op.create_index(
