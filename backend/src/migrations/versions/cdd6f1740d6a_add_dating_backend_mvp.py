...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("avatar_status", sa.String(length=32), nullable=True)
    )
    op.add_column(
        "users",
        sa.Column("avatar_rejection_reason", sa.String(length=255), nullable=True),
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "user_id", "batch_date", name="uq_recommendation_batches_user_date"
        ),
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["batch_id"], ["recommendation_batches.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["target_user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "batch_id", "rank", name="uq_recommendation_items_batch_rank"
        ),
        sa.UniqueConstraint(
            "batch_id", "target_user_id", name="uq_recommendation_items_batch_target"
        ),
    )
    op.create_index(
        "ix_recommendation_items_batch_id",
        "recommendation_items",
        ["batch_id"],
        unique=False,
    )
    op.create_index(
        "ix_recommendation_items_target_user_id",
        "recommendation_items",
        ["target_user_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["serve_item_id"], ["recommendation_items.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["target_user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "actor_user_id",
            "client_event_id",
            name="uq_interaction_events_actor_client",
        ),
    )
    op.create_index(
        "ix_interaction_events_actor_target",
        "interaction_events",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_recommendation_items_target_user_id", table_name="recommendation_items"
    )
