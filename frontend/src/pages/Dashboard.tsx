...
export default function DashboardPage() {
  ...
  const { userSummary, registrations, funnelSummary, funnelDaily, isLoading, isError } =
    useAdminDashboard();
  const location = useLocation();
  const navigate = useNavigate();

  const isAdmin = profile?.roles.includes("admin") ?? false;
  const tab = location.pathname.endsWith("/moderation") ? "moderation" : "overview";
  ...
  return (
    <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 lg:px-6">
      <div className="grid gap-6">
        <section className="relative overflow-hidden rounded-[32px] border border-border bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.22),_transparent_35%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(250,250,249,0.92))] p-6 shadow-sm">
          ...
          <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <Badge variant="outline" className="mb-4 border-amber-300/60 bg-white/80 text-amber-700">
                Admin analytics
              </Badge>
        ...
        <Tabs
          value={tab}
          onValueChange={(value) => navigate(value === "moderation" ? "/dashboard/moderation" : "/dashboard")}
          className="gap-6"
        >
          <TabsList variant="line" className="w-full justify-start border-b border-border pb-3">
            <TabsTrigger value="overview">{t("admin.overview_tab")}</TabsTrigger>
            <TabsTrigger value="moderation">{t("admin.moderation_tab")}</TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        {tab === "moderation" ? (
          <Card className="rounded-[28px] border-dashed bg-muted/25">
            <CardHeader>
              ...
              <CardDescription>{t("admin.moderation_description")}</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricCard
                ...
                hint={t("admin.match_rate", {
                  value: formatPercent(funnelSummary?.conversions.match_rate_from_likes),
                })}
              />
              <MetricCard
                icon={MessageCircleMore}
                title={t("admin.first_replies")}
                value={formatNumber(funnelSummary?.totals.chat_first_reply_received)}
                hint={t("admin.reply_rate", {
                  value: formatPercent(funnelSummary?.conversions.first_reply_rate_from_first_messages),
                })}
              />
              <MetricCard
                icon={ShieldAlert}
                ...
                hint={t("admin.negative_rate", {
                  value: formatPercent(funnelSummary?.conversions.negative_outcome_rate_from_matches),
                })}
              />
            </section>
                    ...
                    <AreaChart data={dailyRows} margin={{ left: 8, right: 8, top: 12 }}>
                      <defs>
                        <linearGradient id="registrationsFill" x1="0" y1="0" x2="0" y2="1">
                          ...
                          <stop offset="95%" stopColor="var(--color-registrations)" stopOpacity={0.03} />
                        </linearGradient>
                        <linearGradient id="feedServedFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="var(--color-feedServed)" stopOpacity={0.3} />
                          <stop offset="95%" stopColor="var(--color-feedServed)" stopOpacity={0.03} />
                        </linearGradient>
                      </defs>
                      <CartesianGrid vertical={false} />
                      <XAxis dataKey="day" tickFormatter={formatDayLabel} tickLine={false} axisLine={false} />
              ...
              <Card className="rounded-[28px]">
                ...
                <CardContent className="grid gap-3">
                  ...
                  <QualityItem
                    ...
                    value={formatPercent(funnelSummary?.conversions.first_message_rate_from_matches)}
                  />
                  <QualityItem
                  ...
                  <QualityItem
                    ...
                    value={formatNumber(funnelSummary?.totals.feed_explanation_opened)}
                  />
                  <QualityItem
                ...
                <CardContent>
                  <ChartContainer config={overviewChartConfig} className="h-[320px] w-full">
                    <BarChart data={dailyRows} margin={{ left: 8, right: 8, top: 12 }}>
                      ...
                      <YAxis tickLine={false} axisLine={false} />
                      <ChartTooltip content={<ChartTooltipContent />} />
                      <Bar dataKey="matches" fill="var(--color-matches)" radius={[8, 8, 0, 0]} />
                      <Bar dataKey="firstMessages" fill="var(--color-firstMessages)" radius={[8, 8, 0, 0]} />
                      <Bar dataKey="replies" fill="var(--color-replies)" radius={[8, 8, 0, 0]} />
                      <Bar dataKey="safetyEvents" fill="var(--color-safetyEvents)" radius={[8, 8, 0, 0]} />
                    </BarChart>
                  </ChartContainer>
                </CardContent>
              </Card>
...
function SegmentCard({
  ...
  items: Array<{
    ...
    counts: { feed_served: number; match_created: number; chat_first_reply_received: number };
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge variant="outline">{formatPercent(item.conversions.first_reply_rate_from_first_messages)}</Badge>
              </div>
              <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                <div>
                  <div className="font-semibold text-foreground">{formatNumber(item.counts.feed_served)}</div>
                  <div>Feed</div>
                </div>
                <div>
                  <div className="font-semibold text-foreground">{formatNumber(item.counts.match_created)}</div>
                  <div>Matches</div>
                </div>
                <div>
                  <div className="font-semibold text-foreground">{formatNumber(item.counts.chat_first_reply_received)}</div>
                  <div>Replies</div>
                </div>
              </div>
            </div>
...
function SegmentMatrixCard({
  ...
  items: Array<{
    user_source: string | null;
    decision_mode: string | null;
    counts: { match_created: number; chat_first_message_sent: number; user_blocked: number; user_reported: number };
  }>;
}) {
  return (
    <Card className="rounded-[28px]">
          ...
          >
            <div>
              <div className="text-sm font-medium">
                {(item.user_source ?? "unknown").replace("_", " ")} x {(item.decision_mode ?? "unknown").replace("_", " ")}
              </div>
              <div className="mt-1 text-xs text-muted-foreground">
                {formatNumber(item.counts.match_created)} matches / {formatNumber(item.counts.chat_first_message_sent)} first messages
              </div>
            </div>
            <Badge variant="outline">
              {formatNumber(item.counts.user_blocked + item.counts.user_reported)} safety
            </Badge>
          </div>
        ))}
      </CardContent>
